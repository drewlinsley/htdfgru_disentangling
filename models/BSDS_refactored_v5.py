...
def build_model(
    ...
    with tf.variable_scope('contour_readout', reuse=reuse):
        h2s = []
        # for layer in layers:
        #     h2s += [tf.image.resize_nearest_neighbor(images=layer, size=h2.get_shape().as_list()[1:3], align_corners=True)]
        # h2 = tf.concat([h2] + h2s, -1)
        activity = normalization.apply_normalization(
        ...
        beta = tf.get_variable(
            name='hello_instance_beta',
            shape=[1, 1, 1, h2.get_shape().as_list()[-1]],
            initializer=tf.initializers.zeros,
            trainable=input_training)
        gamma = tf.get_variable(
            name='helo_instance_gamma',
        ...
        activity = tf.layers.conv2d(
            inputs=activity,
            filters=64,  # 256  # gammanet_constructor[0]['features'],
            kernel_size=ff_kernel_size,
            padding='same',
            data_format=long_data_format,
            name='readout_conv',
        ...
        activity = tf.layers.conv2d(
            inputs=activity,
            filters=64,  # 128,  # gammanet_constructor[0]['features'],
            kernel_size=ff_kernel_size,
            padding='same',
            data_format=long_data_format,
            name='readout_conv_1',
        ...
        #     trainable=training,
        #     reuse=reuse)
    with tf.variable_scope('contour_readout_final', reuse=reuse):
        activity = tf.layers.conv2d(
            inputs=activity,
