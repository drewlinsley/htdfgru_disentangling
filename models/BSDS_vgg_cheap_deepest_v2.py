...
from layers.recurrent import recurrent_vgg16_cheap_deepest as vgg16
...
def v2_big_working():
    compression = ['pool', 'pool', 'pool', 'pool', 'pool', 'upsample', 'upsample', 'upsample', 'upsample']
    ff_kernels = [[False]] * len(compression)
    ff_repeats = [[False]] * len(compression)
    features = [64, 128, 256, 512, 512, 512, 256, 128, 64]  # Default
    # features = [24, 18, 18, 48, 64, 48, 18, 18, 24]  # Bottleneck
    # features = [16, 18, 20, 48, 20, 18, 16]  # DEFAULT
    fgru_kernels = [[3, 3], [3, 3], [3, 3], [3, 3], [3, 3], [1, 1], [1, 1], [1, 1], [1, 1]]
    ar = ['']  # , 'fgru_3', 'fgru_4']  # Output layer ids
    return compression, ff_kernels, ff_repeats, features, fgru_kernels, ar
