...
def build_model(
    ...
    with tf.variable_scope('vgg', reuse=reuse):
        ...
        activity = vgg(rgb=data_tensor, constructor=gammanet_constructor)
    ...
    with tf.variable_scope('readout', reuse=reuse):
        ...
        prepool_activity = tf.layers.conv2d(
            inputs=activity,
            filters=25,  # output_shape,  # int(activity.get_shape()[-1]),
            kernel_size=1,
        ...
        prepool_activity = tf.layers.conv2d(
            ...
            trainable=training,
            use_bias=True)
        out_activity = tf.reduce_mean(prepool_activity, reduction_indices=[1, 2])
        """
        out_activity = tf.reduce_mean(activity, reduction_indices=[1, 2])
    ...
    # extra_activities = {'x': vgg.fgru_0, 'grad_o_r': tf.gradients(out_activity, vgg.fgru_0)[0], 'grad_r_ff': tf.gradients(vgg.fgru_0, vgg.conv2_1)[0]}
    extra_activities = {}
