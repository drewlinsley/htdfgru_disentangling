...
def build_model(
    ...
    with tf.variable_scope('vgg', reuse=reuse):
        ...
        phase_moments_file = "../undo_bias/neural_models/linear_moments/INSILICO_BSDS_vgg_gratings_simple_phase_feature_matrix.npz"
        phase_model_file = "../undo_bias/neural_models/linear_models/INSILICO_BSDS_vgg_gratings_simple_phase_model.joblib.npy"
        ...
        phase_moments = np.load(phase_moments_file)
        phase_means = phase_moments["means"]
        phase_stds = phase_moments["stds"]
        phase_clf = np.load(phase_model_file)  # .astype(np.float32)
        phase_clf = np.maximum(np.minimum(phase_clf, 2^16), 1e-16).astype(np.float32)
        ...
        phase_inv_clf = tf.linalg.inv(tf.matmul(phase_clf, phase_clf, transpose_a=True))
        phase_activity = tf.matmul(
            tf.matmul(phase_inv_clf, phase_clf, transpose_b=True), sel_units, transpose_b=True)

    activity = tf.concat([activity, phase_activity], 0)
    activity = tf.transpose(activity)
    ...
    extra_activities = {"fgru": vgg.fgru_0, "conv": vgg.error_0}  # tf.get_variable(name="perturb_viz")}  # idx: v for idx, v in enumerate(hs_0)}
