...
from layers.recurrent import recurrent_vgg16_cheap_deepest_simple_old as vgg16
    ...
    with tf.variable_scope('fgru', reuse=reuse):
        ...
        for idx, res in enumerate(h2_rem):
            ...
            res = normalization.apply_normalization(
                ...
                reuse=reuse)
            """
            reses += [aux['image_resize'](
                res,
                data_tensor.get_shape().as_list()[1:3],
                align_corners=True)]

        cat_reses = tf.concat(reses, -1)
        """
        cat_reses = tf.layers.conv2d(
            cat_reses,
            filters=cat_reses.get_shape().as_list()[-1],
            kernel_size=(1, 1),
            padding='same',
