...
def build_model(
    ...
    with tf.variable_scope('gammanet', reuse=reuse):
        ...
        gn = gammanet.GN(
            ...
            up_kernel=[4, 4],
            train=training)
        h2, h2_acts = gn(X=activity, store_activities=True)
        # h2 = gn(X=activity)
    ...
    with tf.variable_scope('cv_readout', reuse=reuse):
        ...
        activity = tf.layers.conv2d(
            ...
            reuse=reuse)
    new_h2_acts = []
    with tf.variable_scope('cv_readout', reuse=tf.AUTO_REUSE):
        for res in h2_acts:
    ...
    extra_activities = {'h2_list': tf.concat(new_h2_acts, 0), 'ff': activity}
