...
def build_model(
    ...
    # grads = []
    # for act in h2_acts:
    #     grads += [tf.gradients(act, data_tensor)[0]]
    # extra_activities = {'h2_list': tf.concat(grads, 0), 'ff': vgg.conv2_2}
    new_h2_acts = []
    with tf.variable_scope('fgru', reuse=tf.AUTO_REUSE):
        for res in h2_acts:
            res = normalization.apply_normalization(
                activity=res,
                name='output_norm1_%s' % idx,
            ...
            res = tf.layers.conv2d(
                ...
                reuse=reuse)
            new_h2_acts += [res]
    extra_activities = {'h2_list': tf.concat(new_h2_acts, 0), 'ff': vgg.conv2_2}
