...
class data_processing(object):
    def __init__(self):
        self.name = 'full_LMD_512_egfr'
        self.output_name = 'full_LMD_512_egfr'
        ...
        self.config = Config()
        self.im_size = [2000, 2000, 3]  # 600, 600
        self.model_input_image_size = [2000, 2000, 3]  # [107, 160, 3]
        self.max_ims = 125000
        self.output_size = [4]
    ...
    def get_data(self):
        ...
        all_kras_labels = np.array(
            [1
                if x.split(os.path.sep)[-1].split('_')[0] == 'KRAS'
                else 0 for x in all_ims])
        all_egfr_labels = np.array(
            [1
                if x.split(os.path.sep)[-1].split('_')[0] == 'EGFR'
                else 0 for x in all_ims])
        all_nokras_labels = np.array(
            [1
                if x.split(os.path.sep)[-1].split('_')[0] == 'NOKRAS'
                else 0 for x in all_ims])
        all_noegfr_labels = np.array(
            [1
                if x.split(os.path.sep)[-1].split('_')[0] == 'NOEGFR'
                else 0 for x in all_ims])
        all_labels = np.concatenate(
            (
                all_kras_labels.reshape(-1, 1),
                all_egfr_labels.reshape(-1, 1),
                all_nokras_labels.reshape(-1, 1),
                all_noegfr_labels.reshape(-1, 1)), axis=-1)
        ...
        pos_examples = train_labels[:, 0].sum()
        imbalance = pos_examples - (len(train_labels) - pos_examples)
        if imbalance > 0:
            neg_files = train_ims[train_labels[:, 0] == 0]
            neg_labels = train_labels[train_labels[:, 0] == 0]
            train_ims = np.concatenate((train_ims, neg_files))
            train_labels = np.concatenate((train_labels, neg_labels))
        else:
            pos_files = train_ims[train_labels[:, 0] == 1]
            pos_labels = train_labels[train_labels[:, 0] == 1]
