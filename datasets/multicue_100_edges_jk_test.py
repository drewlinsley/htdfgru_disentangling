...
class data_processing(object):
    def __init__(self):
        self.output_name = 'multicue_100_edges_jk'
        ...
        self.config = Config()
        self.train_size = int(760 * 1)
        self.im_size = [500, 500, 3]  # [321, 481, 3]
        self.model_input_image_size = [500, 500, 3]  # [224, 224, 3]
        self.val_model_input_image_size = [500, 500, 3]
    ...
    def get_files(self):
        ...
        train_labels = [x.replace(self.im_extension, '.edges.npy').replace('images', 'groundTruth') for x in train_images]
        val_images = glob(os.path.join(self.val_images_dir, '*%s' % self.im_extension))
        val_labels = [x.replace(self.im_extension, '.edges.npy').replace('images', 'groundTruth') for x in val_images]
        ...
        val_labels = np.array(
            [np.load(x) for x in val_labels])
        val_images = np.stack([misc.imread(x) for x in val_images], 0)
        val_labels = np.stack(val_labels, 0)[..., None]

        # Build CV dict
        cv_files, cv_labels = {}, {}
        cv_files[self.folds['train']] = train_images
        cv_files[self.folds['val']] = val_images
        cv_files[self.folds['test']] = val_images
    ...
    def get_test_files(self):
        ...
        test_images = np.array(
            glob(os.path.join(self.val_images_dir, '*.jpg')))
        return test_images
