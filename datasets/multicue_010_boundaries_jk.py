...
class data_processing(object):
    def __init__(self):
        self.output_name = 'multicue_010_boundaries_jk'
    ...
    def get_files(self):
        ...
        train_labels = [x.replace(self.im_extension, '.boundaries.npy').replace('images', 'groundTruth') for x in train_images]
        val_images = glob(os.path.join(self.val_images_dir, '*%s' % self.im_extension))
        val_labels = [x.replace(self.im_extension, '.boundaries.npy').replace('images', 'groundTruth') for x in val_images]
