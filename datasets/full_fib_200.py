...
class data_processing(object):
    def __init__(self):
        self.name = 'full_fib_200'
        self.output_name = 'full_fib_200'
        self.contour_dir = '/media/data_cifs/connectomics/datasets/fib25_0.npz'
        self.config = Config()
    ...
    def get_data(self):
        ...
        volume = d['volume'][:512, :512, :512]
        label = d['label'][:512, :512, :512]
        strides = np.arange(0, volume.shape[1], self.stride_size)[:-1]
        window_volume, window_label = [], []
        for vol, lab in zip(volume, label):
            for x_stride_start in strides:
                x_stride_end = x_stride_start + self.im_size[1]
                for y_stride_start in strides:
                    y_stride_end = y_stride_start + self.im_size[0]
                    if (y_stride_end > volume.shape[1]) or (
                            x_stride_end > volume.shape[2]):
                        continue
                    else:
                        print (y_stride_end, x_stride_end)
                        window_volume += [
                            vol[
        ...
        # Balance CV folds
        val_cutoff = -10  # np.round(num_ims * self.cv_split).astype(int)
