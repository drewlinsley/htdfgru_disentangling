...
class data_processing(object):
    def __init__(self):
        self.name = 'orientation_tilt'
        # self.name = 'plaid_surround'
        self.output_name = 'orientation_tilt_viz'
        self.img_dir = 'imgs'
        ...
        self.perturb_a = "../refactor_gammanet/plaid_surround_outputs/BSDS_vgg_gratings_simple_plaid_surround_2020_09_27_10_13_50_541685.npz"
        self.perturb_b = "perturb_viz/gammanet_full_plaid_surround_outputs_data.npy"
    ...
    def get_data(self):
        ...
        # Get images
        test_images = np.stack([misc.imread(x) for x in test_images], 0).astype(np.float32)

        # Modulate
        modulate = 10
        test_images //= modulate  # Used to just be /
        ...
        # Get targets
        """
        targets = np.load("perturb_viz/BSDS_vgg_gratings_simple_BSDS500_test_orientation_2020_08_26_14_09_43_696359.npz", allow_pickle=True)
        ...
        logits_a = np.load(self.perturb_a, allow_pickle=True)
        logits_a = logits_a["test_dict"]
        logits_a = np.asarray([x["prepre_ephys"] for x in logits_a])  # Changed from ephys 11/9/20
        logits_a = logits_a.squeeze()  # T
        print("Extracting tuning curves for targets.")

        # Logits for target
        logits_b = np.load(self.perturb_b, allow_pickle=True)
        logits_b = logits_b.T
        if len(logits_b) == 181:
            logits_b = logits_b[1:]
        print("Extracting tuning curves for targets.")
