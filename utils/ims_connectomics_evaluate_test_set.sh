...
declare -a EXPERIMENTS=("test_full_snemi_combos" "full_berson_combos" "full_fib_combos" "full_cremi_combos") 
...
do
    ...
    for (( i=0; i<${#array1[@]}; i++ ));
    do
        echo "Beginning $i, command: CUDA_VISIBLE_DEVICES=$GPU python run_job.py --experiment=$experiment --ckpt=${array1[$i]} --model=${array2[$i]} --add_config=${array3[$i]} --out_dir=$OUTPUT --num_batches=$IM_BATCHES --map_out=$MAP_OUT --placeholders --test --no_db --no_npz"
        CUDA_VISIBLE_DEVICES=$GPU python run_job.py --experiment=$experiment --ckpt=${array1[$i]} --model=${array2[$i]} --add_config=${array3[$i]} --out_dir=$OUTPUT --num_batches=$IM_BATCHES --map_out=$MAP_OUT --placeholders --test --no_db --no_npz
    done
done
