...
class hGRU(object):
    ...
    def full(self, i0, x, h1, h2, inh, var_scope='hgru_weights'):
        """hGRU body."""
        ...
        a = self.kappa * (h2 + e)
        m = self.omega * (h2 * e)
        h1 = self.recurrent_nl(a + m)

        # Circuit output receives recurrent input h1
        c1, g2 = self.circuit_output(var_scope=var_scope, h1=h1)

        # Calculate input (-) integration: h1 (4)
        if i0 == self.timesteps - 1:
            inh = (self.alpha * h1 + self.mu) * c1
            return i0, x, h1, h2, inh
        h2_hat, inh = self.input_integration(
            x=x,
            c1=c1,
            h2=h1)
        h2 = (g2 * h2) + ((1 - g2) * h2_hat)
    ...
    def condition(self, i0, x, h1, h2, inh):
