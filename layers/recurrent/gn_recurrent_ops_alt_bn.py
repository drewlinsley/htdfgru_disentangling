...
class GNRnOps(object):
    ...
    def output_integration(self, h1, c2, g2, h2, layer_id, perturb_function=None):
        ...
        if self.multiplicative_excitation:
            ...
            # h2_hat = self.recurrent_nl((kappa * h1 + omega) * c2 + h1)
            a = kappa * (h1 + c2)
            m = omega * (h1 * c2)
            if perturb_function is not None:
                h2_hat = self.recurrent_nl(a + m) + tf.nn.relu(-self.mult)
            else:
