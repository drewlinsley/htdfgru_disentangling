    ...
    def __call__(self, rgb, label, constructor=None, store_timesteps=False):
        ...
        if self.perturb is not None:
            ...
            if 0:  # self.perturb <= 1:
                ...
                perturbation = tf.cos((np.pi * (orientations - (b + mu)) / 180))
                perturbation = tf.nn.relu(perturbation)
                perturbation = self.perturb * (perturbation ** a)

                # bin here
                binned_perturbation = tf.reduce_mean(tf.reshape(perturbation, [6, -1]), -1)
            ...
            if self.perturb_method == "kernel":
                ...
                self.fgru_0 = (mult * self.conv2_2 * perturb_mask + perturb_bias) + tf.stop_gradient(1 - perturb_mask)
