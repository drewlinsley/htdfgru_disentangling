...
def old_pinv(matrix, eps=1e-5):  # 1e-8
...
class Vgg16(GN, CreateGNParams, GNRnOps, GNFFOps):
    ...
    def __call__(self, rgb, label, constructor=None, store_timesteps=False):
        ...
        if self.perturb is not None:
            ...
            if 1:  # self.perturb != 1:  # self.perturb != 1:  # self.perturb <= 1:
                ...
                M = tf.matmul(tf.matmul(B, A, transpose_b=True), linalg.pinv(Z, rcond=self.rcond))
            ...
            if 1:  # self.perturb != 1:
                ...
                inv_inv = np.linalg.pinv(clf.dot(clf.T), rcond=self.rcond)
                ...
                tc_inv = tf.nn.relu(tf.reshape(tc_inv, [-1]))
                self.tc_inv = tc_inv
                if self.perturb_norm:
            ...
            if self.perturb_method == "kernel":
                ...
                #### THIS WORKS
                self.fgru_0 = self.conv2_2 * perturb_mask + perturb_bias  # * perturb_mask + perturb_bias  #  + perturb_bias  # * tf.identity(self.conv2_2) + perturb_bias
                # self.fgru_0 = self.fgru_0 * perturb_mask + tf.stop_gradient((1 - perturb_mask) * self.fgru_0)
                self.mult = mult * perturb_mask + tf.stop_gradient((1 - perturb_mask) * mult) # Attach so we can penalize
                self.perturb_bias = perturb_bias
                # self.mult = self.mult  #  + perturb_bias

                # Fix the FF drive!
                # self.conv2_2 = tf.stop_gradient(self.conv2_2) * perturb_mask - perturb_bias  # tf.nn.relu(perturb_bias)
                self.conv2_2 = self.conv2_2 * perturb_mask + perturb_bias  # tf.nn.relu(perturb_bias)
                self.conv2_2 = tf.nn.relu(self.conv2_2)
                if 0:  # use_mask:
                    # Mask the BG
                    # self.fgru_0 = bg * self.fgru_0 + tf.stop_gradient((1 - bg) * self.fgru_0)
                    self.mult = bg * self.mult + tf.stop_gradient((1 - bg) * self.mult)
                else:
                    ...
                    self.mult = mask * self.mult + tf.stop_gradient((1 - mask) * self.mult)
                    self.mask = mask
            else:
    ...
    def build(self, i0, extra_convs=True):
        ...
        with tf.variable_scope('fgru'):
            if 1:  # i0 == 0:  # Add a control variable to correct perturbation
